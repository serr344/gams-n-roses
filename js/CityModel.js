    ...
    findOptimalStageLocation() {
        ...
        for (let x = 15; x < GS - 30; x += 15) {
            for (let y = 15; y < GS - 30; y += 15) {
                ...
                if (!this.isClearForPark(x, y, 20, 20)) continue;

                let currentViolations = 0;
                let penalty = 0;

                // LP: maxDb = min_i( dbLimit_i + 20*log10(d_i) )
                let maxDb = Infinity;

                for (let b of this.buildings) {
                    const dist = Math.hypot(b.centerX - x, b.centerY - y);
                    
                    if (dist < 1 || dist > 150) continue;

                    const buildingDb = b.dbLimit || b.db;
                    const receivedDB = 92 - 20 * Math.log10(dist);
                    const allowed = buildingDb + 20 * Math.log10(dist);

                    if (allowed < maxDb) maxDb = allowed;

                    if (receivedDB > buildingDb) {
                        currentViolations++;
                        penalty += (receivedDB - buildingDb) * 400;
                        penalty += 2500;
                    }
                }

                let currentScore = 18000 - penalty;
                ...
                if (currentScore > bestScore) {
                    ...
                    bestLocation = { 
                        x: x, 
                        y: y, 
                        maxSeyirci: Math.max(0, Math.floor(currentScore)), 
                        ihlal: currentViolations,
                        maxDb: maxDb === Infinity ? 130.0 : Math.round(maxDb * 10) / 10
                    };
                }
            }
        }
