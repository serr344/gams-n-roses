// ═══════════════════════════════════════════════════════════════════
// GAMS N' ROSES — stage.js (COIN ECONOMY, AUDIENCE & NAVIGATION)
// ═══════════════════════════════════════════════════════════════════
...
SLOT_IDS.forEach(id => { placedItems[id] = null; });

// ── INIT ───────────────────────────────────────────────────────────
window.addEventListener('DOMContentLoaded', () => {
  const raw = localStorage.getItem('stageOptimData');
  if (raw) {
    try { stageData = JSON.parse(raw); } catch(e) {}
  }
...
function buildSlots() {
  ...
  SLOT_IDS.forEach(id => {
    ...
    div.onclick = (e) => { e.stopPropagation(); onSlotClick(id, div, e); };
...
function onSlotClick(slotId, el, e) {
  if (selectedItem) {
...
function attemptPlaceItem(slotId, item) {
  ...
  if (oldItem) { requiredCoins -= oldItem.cost; }
...
function itemContribution(item, slotAngle, targetAngle) {
  ...
  if (item.type === 'hybrid' && item.reflectMagnitude) {
    ...
    effect += item.reflectMagnitude * Math.exp(-(reflDiff * reflDiff) / (2 * item.reflectSpread * item.reflectSpread));
  }
  return effect;
}
...
function calculateCrowd() {
  ...
  // +450 audience per 1 dB above 50 dB
  const baseCrowd = Math.max(0, Math.floor((adb - 50) * 450));
  
  // -2500 audience penalty per violated building (ticket cancellation)
...
function updateHeader() {
  ...
  // Format audience count with thousands separator (e.g. 15,200)
  ...
  // Turn audience count red if there are violations (warning)
...
function updateScorePanel() {
  ...
  if(bar) {
    ...
    bar.style.width = pct + '%';
    bar.style.background = dbColor(adb).replace('var(--','').replace(')','') === 'green'
      ? '#39ff14' : adb <= 85 ? '#ffc300' : '#ff1744';
  }
}
...
function updateBuildingList() {
  ...
  if (!stageData?.data?.nearby || stageData.data.nearby.length === 0) {
    list.innerHTML = '<div style="padding:20px 14px;text-align:center;font-size:12px;color:var(--dim)">No buildings in range</div>';
    return;
  }

  const sorted = [...stageData.data.nearby].sort((a, b) => {
    const ra = dbReceivedAt(a), rb = dbReceivedAt(b);
    const va = ra > a.dbLimit, vb = rb > b.dbLimit;
    if (va && !vb) return -1;
    if (!va && vb) return 1;
  ...
  sorted.forEach(b => {
    ...
    const margin   = (b.dbLimit - received).toFixed(1);
    const color    = violated ? 'var(--red)' : received > b.dbLimit - 5 ? 'var(--gold)' : 'var(--green)';
    ...
    row.innerHTML = `
      ...
      <div>
        <div class="bld-recv" style="color:${color}">${received.toFixed(1)}</div>
        <div class="bld-limit">${violated ? '⚠ +'+Math.abs(margin) : '✓ −'+Math.abs(margin)} dB</div>
      </div>`;
...
function drawSoundField(ctx, cx, cy) {
  ...
  for (let i = 0; i < SECTORS; i++) {
    ...
    const modifier = totalModifierAt(sectorAngle);
    const dbAtRef  = BASE_DB + modifier - 20 * Math.log10(R_REF);
  ...
  ctx.lineWidth = 1.5;
  ctx.setLineDash([8, 5]);
  ctx.beginPath(); ctx.arc(cx, cy, R_MAX, 0, Math.PI * 2); ctx.stroke();
  ctx.setLineDash([]);

  [0.33, 0.66].forEach(f => {
    ctx.strokeStyle = 'rgba(255,195,0,0.07)';
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.arc(cx, cy, R_MAX * f, 0, Math.PI * 2); ctx.stroke();
  });
}
...
function drawStage(ctx, cx, cy, sw, sh) {
  ...
  ctx.beginPath(); ctx.arc(cx, cy - sh * 0.25, 4, 0, Math.PI * 2); ctx.fill();
...
function drawItemArrows(ctx, cx, cy, sw, sh) {
  SLOT_IDS.forEach(id => {
    ...
    const compassAngle = SLOT_DEFS[id].angle;
    const canvasAngle = (compassAngle - 90) * Math.PI / 180;

    const gap = 38 + (id.length === 2 ? Math.max(sw, sh) / 2 + 10 : (id === 'N' || id === 'S' ? sh / 2 + 10 : sw / 2 + 10));
    ...
    ctx.strokeStyle = isBarrier ? item.color : item.color;
...
window.showResults = function() {
  ...
  const itemCount = SLOT_IDS.filter(id => placedItems[id]).length;
  if (itemCount === 0) {
    alert("You haven't placed any stage equipment yet! Place at least one item before comparing.");
    return;
  }
  ...
  const violations = countViolations();
  if (violations > 0) {
    alert(`You are currently violating ${violations} building limit(s)! Resolve all violations before viewing results.`);
    return;
  }

  // Save user's score to localStorage
  localStorage.setItem('gams_userCrowd', userCrowd);
